...
class GridWorldMDP1(MDP):
    ...
    def __init__(self,
        ...
        MDP.__init__(self, GridWorldMDP1.ACTIONS, self._transition_func, self._reward_func, init_state=init_state, gamma=gamma)
...
def _error_check(state, action):
    ...
    if action not in GridWorldMDP1.ACTIONS:
...
def make_grid_world_from_file(file_name, randomize=False, num_goals=1, name=None, goal_num=None, slip_prob=0.0):
    ...
    return GridWorldMDP1(width=num_cols, height=num_rows, init_loc=(agent_x, agent_y), goal_locs=goal_locs, lava_locs=lava_locs, walls=walls, name=name, slip_prob=slip_prob)
...
def main():
    grid_world = GridWorldMDP1(5, 10, (1, 1), (6, 7))
