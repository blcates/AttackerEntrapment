...
class Defender(MDP):
    ...
    """
    def __init__(self, attacker, attacker_planner, k=10, step_cost=0):
        self.name = "Defender"
        self.budget = k
    ...
    def _reward_func(self, state, action, next_state):
        ...
        if valid_move and valid_move2 and (next_state.x, next_state.y) in self.goal_locs:
            defender_reward = 0
    ...
    def _transition_func(self, defender_state, defender_action):
        ...
        if (defender_next_state.x, defender_next_state.y) in self.goal_locs or defender_next_state.budget == 0:
