...
class GridWorldMDP3(MDP):
    ...
    def __init__(self,
        ...
        MDP.__init__(self, GridWorldMDP3.ACTIONS, self._transition_func, self._reward_func, init_state=init_state, gamma=gamma)
    ...
    def _transition_func(self, state, action):
        ...
        if not(self._is_goal_state_action(state, action)) and self.slip_prob > random.random():
            # Flip dir.
            if action == "up":
                action = random.choice(["up", "left", "right"])#["left", "right"])
            elif action == "down":
                action = random.choice(["up", "left", "right"])#["left", "right"])
            elif action == "left":
                action = random.choice(["up", "left", "right"])#["left", "right"])
...
def _error_check(state, action):
    ...
    if action not in GridWorldMDP3.ACTIONS:
...
def make_grid_world_from_file(file_name, randomize=False, num_goals=1, name=None, goal_num=None, slip_prob=0.0):
    ...
    return GridWorldMDP3(width=num_cols, height=num_rows, init_loc=(agent_x, agent_y), goal_locs=goal_locs, lava_locs=lava_locs, walls=walls, name=name, slip_prob=slip_prob)
...
def main():
    grid_world = GridWorldMDP3(5, 10, (1, 1), (6, 7))
