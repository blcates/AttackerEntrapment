...
class PuddleMDP(GridWorldMDP):
    ...
    def _transition_func(self, state, action):
        ...
        noise_list = []
        delta_diff = self.delta/4.0
        curr_dist = 0.0
        while curr_dist <= self.delta:
            curr_dist +=  delta_diff
            noise_list.append(curr_dist)
        #noise = np.random.choice([-0.2,-0.1,0.1,0.2])#randn(1)[0] / 100.0
        noise = np.random.choice(noise_list)
        #to_move = self.delta + noise
        #to_move = noise
        to_move = self.delta

        if not(self.is_loc_within_radius_to_goal(state.x,state.y)) and self.slip_prob > random.random():
            # Flip dir.
            if action == "up":
